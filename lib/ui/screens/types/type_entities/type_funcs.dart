...
import 'package:pokedex_demo/ui/screens/types/type_entities/type_constants.dart';
...
List<String> normalTypeReturner(List<String> combined) {
  var normalSet =
      typeNames.toSet(); // Create a set from the constant list of types
...
List<String> effectreturner(int i, dynamic n) {
  if (n == 0) {
    ...
    return normalTypeReturner(types[i].nilEffective +
        types[i].superEffective +
        types[i].notEffective);
  } else if (n == 0.5) {
